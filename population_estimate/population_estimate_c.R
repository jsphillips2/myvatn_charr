...
lp <- rbind(c(1.5, 1.5/2000),
            c(4, 4)) 

# phi priors
pp <- rbind(c(1.5, 1.5/0.5 - 1.5),
            c(4, 4))

# sigma priors
rl <- matrix(c(1.5, 1.5/2),
             nrow = 1, ncol = 2,
             byrow = T) 
...
# file path for model
model_path <- "population_estimate/char_pop_est_c.txt"

# variables to monitor
monitor <- c("lambda_h","phi_h","lambda","nu","phi","r","nu_l")
...
fit_sum %>%
  filter(rowname == "lambda_h")

fit_sum %>%
  filter(rowname == "phi_h")
...
bayesplot::mcmc_trace(x = fit$mcmc, pars = "lambda_h")

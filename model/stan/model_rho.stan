...
parameters{
  real log_rho_init; // initial value for log_rho
  real<lower=0> sig_rho; // sd for stochastic change in log_rho
  real z_rho[nYears-2]; // random variate for stochastic change in log_rho
  real logit_phi[nStages-1]; // initial value for logit_phi
...
transformed parameters{
  ...
  real log_rho[nYears-1]; // per captia recruitment (on log scale)
  real rho[nYears-1]; // per recruitment
  real phi[nStages-1]; //survival probability
  real n_pred[nStages, nYears]; // estimated abundance on log scale
  // initial values
  for(i in 1:nStages){
    N[i,1] = N_init[i];
  }
  log_rho[1] = log_rho_init;
  // time-varying parameters
  for(t in 2:(nYears - 1)){
    log_rho[t] = log_rho[t-1] + sig_rho*z_rho[t-1];
  // tranform parameters
  rho = exp(log_rho);
  ...
  for (t in 2:nYears) {
    N[1,t] = scale*rho[t-1]*N[4,t-1];
    N[2,t] = N[1,t-1];
    N[3,t] = phi[1]*N[2,t-1];
    N[4,t] = phi[2]*N[3,t-1] + phi[3]*N[4,t-1];
  }
...
model{
  ...
  z_rho ~ normal(0, 1);
  // priors
  log_rho_init ~ normal(0, 1);
  sig_rho ~ normal(0, 2) T[0,];
...
generated quantities {
  ...
  real rho_scale[nYears-1]; // rho on natural scale
